...
class UpdateStoreUserProfileService {
    constructor(
    ...
    async execute(data: UpdateRequest) {
        ...
        return await prismaClient.$transaction(async (tx) => {
            ...
            if (data.userType === 'STORE_USER') {
                if (data.performedByUserId !== data.id) {
                    throw new ForbiddenError("OnlySelfUpdateAllowed")
                }
            ...
            await this.auditLogService.create({
                action: "STORE_USER_PROFILE_UPDATE",
                details: { 
                    targetUserId: data.id,
                    isSelfUpdate: data.performedByUserId === data.id,
...
export { UpdateStoreUserProfileService };